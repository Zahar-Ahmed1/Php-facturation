...
// Gestion de l'URL pour Apache (.htaccess) ou PHP Built-in Server
$url = isset($_GET['url']) ? $_GET['url'] : parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
$url = ltrim($url, '/');
$url = rtrim($url, '/');
$parts = explode('/', $url);

// Si l'URL commence par 'index.php', on l'enlève
if (isset($parts[0]) && $parts[0] === 'index.php') {
    array_shift($parts);
}

// Si l'URL commence par 'api/', on l'enlève pour le routage
...
// Routes API
error_log("Request URL: " . $url . " | Method: " . $method);
switch ($parts[0]) {
    case '': // Route racine
        if ($method == 'GET') {
    ...
    case 'clients':
        ...
        if ($method == 'GET') {
            ...
            http_response_code(405);
            echo json_encode(["error" => "Méthode non autorisée ou paramètre manquant."]);
        }
        break;
